...
# 2022 May 06 - Sina Hafezi - initial version including all read-in/plotting/
#               processing utilities of audio data & metadata
# 2022 Jul 25 - Sina Hafezi - replacing ht/pos interpolant function with ht/pos
#               sample values (while still enabling interpolation,
#               main interpolation moved to Processor class)
# 2022 Jul 28 - Sina Hafezi - making the paths concatenation robust to
#               operating sys using os.path
...
class SPEAR_Data:
    ...
    total_frames = file_duration * fps  # number of OptiTrack frames per file
    total_sources = 7                   # total number of possible participants
    wearer_ID = 2                       # participant ID wearing the array

    # main root path to dataset - where 'Dataset_X' folders are subfolders
    root_path = ''
    ...
    atf_file = os.path.join(
        'Miscellaneous', 'Array_Transfer_Functions', 'Device_ATFs.h5')
    ...
    """
    ht:      (ndarray) head-tracking quaternions interpolant function
             (t,qx,qy,qz,qw) as [total_frames x 5]
    t:       (ndarray) OptiTrack time frames in sec [total_frames x 1]
    IDs:     list of participant IDs present in the file
    src_pos: (dict of cartesian position, keys: source ID) cartesian position
             relative to the (rotated) array [total_frames x 4]
             (t,x,y,z) ndarray
    """
    ht = []
    t = []
    ...
    def set_file(obj, dataset, session, filename):
        # DESCRIPTION: set the folders/files for case of interest and
        ...
        obj.ht_file = os.path.join(
            obj.root_path, obj.dataset_folder, obj.ht_folder,
            obj.session_folder, f'ori_{obj.file}.csv')
        obj.array_file = os.path.join(
            obj.root_path, obj.dataset_folder, obj.array_folder,
            obj.session_folder, f'array_{obj.file}.wav')
        obj.vad_file = os.path.join(
            obj.root_path, '..', '..', 'Extra',
            obj.root_path.split(os.sep)[-1], obj.dataset_folder,
            obj.vad_folder, obj.session_folder, f'vad_{obj.file}.csv')
        obj.load_ht()
        obj.set_participant_IDs()
        obj.load_pos()

    def get_all_AIRs(obj):
        # DESCRIPTION: returns dictionary of array's Acoustic Impulse Responses
        #              (AIRs) for all measured directions
        # *** OUTPUTS ***
        # AIR        (dict) dictionary
        #                {'IR': (nSample,nDirection,nChannel),
        #                 'fs': (int), 'directions': (N,2), 'nChan': (int)}
        AIR = {'IR': [], 'fs': [], 'directions': [], 'nChan': [],
               'azi': [], 'ele': []}
        ...
        f = h5py.File(
            os.path.join(obj.root_path, '..', '..', obj.atf_file), 'r')
        # groups = list(f.keys())
        AIR['fs'] = int(list(f['SamplingFreq_Hz'])[0][0])
        # AIR['IR'] (ndarray) [nSample x nDirection x nChan]
        AIR['IR'] = np.array(f['IR'])
        # AIR['ele'] (ndarray) elevation in radians [1 x nDirection]
        AIR['ele'] = (np.pi/2)-np.array(f['Theta'])
        # AIR['azi'] (ndarray) azimuth in radians [1 x nDirection]
        AIR['azi'] = np.array(f['Phi'])
        # AIR['directions'] (ndarray) [nDirection x 2]
        AIR['directions'] = np.concatenate((AIR['azi'], AIR['ele']), axis=0).T
        # AIR['ele'] (ndarray) [nElevation x 1]
        # AIR['azi'] (ndarray) [nAzimuth x 1]
        AIR['ele'] = np.sort(np.unique(AIR['ele']))
        AIR['azi'] = np.sort(np.unique(AIR['azi']))
        AIR['nChan'] = AIR['IR'].shape[-1]
        f.close()
        return AIR

    def get_targets(obj):
        # DESCRIPTION: returns the participant ID of targets
        #              (all sources except ego-centric wearer)
        # *** OUTPUTS ***
        # targets  (list) ID of all potential targets wrt. wearer
        targets = list(np.array(obj.IDs)[
            np.where(np.array(obj.IDs) != obj.wearer_ID)[0]])
        return targets
    ...
    def load_ht(obj):
        # DESCRIPTION: load the head-rotation of the wearer and
        #              initialize holder param
        # data (nFrame, 6) DataFrame [index time qx qy qz qw]
        data = pd.read_csv(obj.ht_file)
        data = data.to_numpy()
    ...
    def set_participant_IDs(obj):
        ...
        path = os.path.join(
            obj.root_path, obj.dataset_folder, obj.doa_folder,
            obj.session_folder)
    ...
    def load_pos(obj):
        # DESCRIPTION: load cartesian position of present sources
        #              and initialize holder param (dict)
        obj.src_pos = dict.fromkeys(np.array(obj.IDs))
        file = os.path.join(
            obj.root_path, obj.dataset_folder, obj.doa_folder,
            obj.session_folder, f'doa_{obj.file}_ID')
        for si in range(0, len(obj.IDs)):
            ...
            # (dict of cartesian positions) [t x y z] wrt. to wearer
            obj.src_pos[obj.IDs[si]] = np.array([t, x, y, z]).T   # (nFrame,4)

    def get_doa(obj, src_ID, t):
        # DESCRIPTION: return the doas of a requested participant ID over
        #              requested times
        # *** INPUTS ***
        ...
        pos_interp = interp1d(
            obj.src_pos[src_ID][:, 0], obj.src_pos[src_ID][:, 1:],
    ...
    def get_pos_samples(obj, src_ID):
        # DESCRIPTION: returns the cartesian position samples of requested
        #              participant ID
        # *** INPUTS ***
        # src_ID  (int) participant ID of interest
        # *** OUTPUTS ***
        # (ndarray) relative cartesian positions samples over time
        # [t x y z] (nFrame,4)
        return obj.src_pos[src_ID]

    def rotate_sys(obj, q, point):
        # DESCRIPTION: returns cartesian of a point after rotating
        #              the cartesian system
        # *** INPUTS ***
        # q       (ndarray) rotation quartenions (xyzw) of the system [1 x 4]
        # point   (ndarray) cartesian position (xyz) of a point [1 x 3]
        #                   in default world coordinate system
        # *** OUTPUTS ***
        # point   (ndarray) cartesian position (xyz) of a point [1 x 3]
    ...
    def get_array_audio(obj, fs=None, start_t=0.0, duration=None, mics=[]):
        ...
        y, fs = librosa.load(
            obj.array_file, sr=fs, mono=False,
            offset=start_t, duration=duration)
        if mics:
            y = y[np.array(mics)-1, :]
        return y, fs

    def get_cases(obj, option=[]):
        # DESCRIPTION: returns data frame table of cases
        ...
        # *** INPUTS ***
        # option   (list) [] or ['Dataset'] or ['Dataset',Session]
        #                 or ['Dataset,Session,'File']
        # *** OUTPUTS ***
        # cases    (Pandas DataFrame) row: a case,
        #                             columns: 'dataset', 'session', 'file'
        ...
        for d in datasets:
            path = os.path.join(
                obj.root_path, obj.dataset_prefix+str(d), obj.array_folder)
            if not sessions:
            ...
            for s in sessions:
                if not files:
                    files = []
                    files_paths = glob.glob(os.path.join(
                        path, f'Session_{s}', '*.wav'))
                    files_paths.sort()
                    for file in files_paths:
                        files.append(file.split(
                            os.sep)[-1].split('_M')[-1].split('.wav')[0])
                for f in files:
                    c['dataset'].append(d)
                    c['session'].append(s)
                    c['file'].append(f)
    ...
    def get_VAD(obj):
        ...
        cols = list(data.keys())
        IDs = [n for n in range(1, obj.total_sources+1)
               if any(s for s in cols if str(n) in s)]
        data = data.to_numpy()  # (nFrame,4) [index time azi el] in degress
        for ID in IDs:
    ...
    def plot_VAD(obj):
        ...
        plt.figure()
        plt.pcolormesh(
            t, source_IDs, vads, shading='auto', cmap='binary', vmin=0, vmax=1)
        plt.xlabel('[sec]')
        plt.ylabel('Participant ID')
        plt.grid(linestyle=':')
        plt.title(
            'Dataset: %d, Session: %d, File: %s' %
            (obj.dataset, obj.session, obj.file))
