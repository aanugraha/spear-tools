...
# 2022 May - Sina Hafezi - first version inclusing baseline beamformer
#            (Isotropic-MVDR/Superdirective) & all required utility functions
# 2022 Jul - Sina Hafezi - moved pos/ht interpolation from dataset class here
...
class SPEAR_Processor:
    # (list) channels at which to get enhanced audio (indexing from 1)
    out_chan = [2]
    ...
    # (ndarray) fft of IR [nFreq x nDirection x nChan]
    ...
    # (SciPy 1D-interpolant) cartesian position interpolant function returning
    # array of (x y z) as ndarray for requested time(s)
    target_pos_interp = []
    ...
    R_iso:
        ...
        (used in superdirective beamformer - baseline method)
    w_conj:
        pre-calculated beamforming conjugate weights for all directions
        [nChan x nFreq x nDir x nOut]
    w_stft:
        beamforming conjugate weights in stft domain
        [nChan x nFreq x nFrame x nOut]
    diag_load_mode:
        type of diagonal loading. 'cond': limit maximum condition number.
        'const': add a constant identity. [] to disable diag loading.
    diag_load_val (int):
         max condition number for 'cond' mode,
         constant value for 'const' mode
    """
    R_iso = []
    w_conj = []
    w_stft = []
    diag_load_mode = 'cond'
    diag_load_val = int(1e2)
    ...
    def __init__(obj, air, fs=[], mics=[], out_chan=out_chan):
        ...
        # *** INPUTS ***
        # air      (dict) dictionary of AIR
        #                 (output of get_all_AIRs() in dataset class)
        # fs       (int)  processing sample rate. [] for default
        # mics     (list) list of mic subsets to be used for beamforming
        #                 (indexing from 1). [] for all
        # out_chan (list) list of mic subsets at which the beamforming output
        #                 are obtained (indexing from 1)

        # obj.IR (ndarray) Impulse Responses [nSample x nDirection x nChan]
        obj.IR = air['IR']
        fs0 = air['fs']
        obj.total_sensor = air['nChan']
        # obj.dirs (ndarray) (azimuth, elevation) in radians [nDirection x 2]
        obj.dirs = air['directions']
        ...
        # keeping the IRs for required mics subset
        obj.IR = obj.IR[:, :, list(np.array(obj.mics)-1)]
        nSample, nDir, nChan = obj.IR.shape
        # resampling IRs if needed
        if obj.fs != fs0:
            obj.IR = np.transpose(obj.IR)  # [nChan x nDir x nSample]
            obj.IR = obj.IR.reshape((-1, nSample))  # [nChan*nDir x nSample]
            # [nChan*nDir x nSample2]
            obj.IR = librosa.resample(obj.IR, orig_sr=fs0, target_sr=obj.fs)
            # [nChan x nDir x nSample2]
            obj.IR = obj.IR.reshape((nChan, nDir, -1))
    ...
    def set_out_channels(obj, c):
        ...
        # *** INPUTS ***
        # c   (list) list of mic subsets at which the beamforming output
        #            are obtained (indexing from 1)
        obj.out_chan = c
        obj.nOut = len(c)

    def set_target_pos(obj, target_pos):
        # DESCRIPTION: update the target position interpolant function for
        #              the requested target
        # *** INPUTS ***
        # target_pos   (ndarray) cartesian position over time samples (t x y z)
        #                        as [total_frames x 4]
        obj.target_pos_interp = interp1d(
            target_pos[:, 0], target_pos[:, 1:], kind='previous',
            axis=0, fill_value='extrapolate')

    def set_diag_loading(obj, mode='cond', val=int(1e5)):
        # DESCRIPTION: set the settings for diagonal loading of
        #              covariance matrix
        # *** INPUTS ***
        # mode  (str)    'cond': based on maximum condition limiting.
        #                'const': based on adding a fixed constant
        # val   (float)  maximum condition number if mode=='cond' or
        #                weight of identity matrix added if mode=='const'
    ...
    def calculate_iso_cov(obj):
        # DESCRIPTION: calculates the isotropic diffuse covariance matrix
        #              (for superdirective/Iso-MVDR beamformer)
        # ATF [nFreq x nDir x nChan]
        w_quad = obj.get_quadrature_weights()  # [nDir x 1]
        # [nFreq x nChan x nChan x nDir]
        w_quad = np.matlib.tile(w_quad, (obj.nFreq, obj.nChan, obj.nChan, 1))
        # [nFreq x nDir x nChan x nChan]
        w_quad = np.transpose(w_quad, axes=[0, 3, 1, 2])
        # [nFreq x nDir x nChan x nChan]
        obj.R_iso = obj.ATF[:, :, :, None] @ np.conj(obj.ATF[:, :, None, :])
        obj.R_iso = obj.R_iso * w_quad  # quadrature weighting
        # [nFreq x nChan x nChan]
        obj.R_iso = np.squeeze(np.sum(obj.R_iso, axis=1))
        if obj.diag_load_mode:
            # diag loading is requested
            for fi in range(obj.nFreq):
                obj.R_iso[fi, :, :] = obj.diag_load_cov(
                    np.squeeze(obj.R_iso[fi, :, :]))
    ...
    def diag_load_cov(obj, R):
        ...
        if obj.diag_load_mode == 'const':
            ...
            if cn0 > threshold:
                ...
                R = R + np.eye(obj.nChan) *\
                    (ev.max() - threshold * ev.min()) / (threshold - 1)
        return R

    def get_quadrature_weights(obj):
        # DESCRIPTION: returns the quadrature weights for the elevations
        # NOTE: These weights are needed to compensate for higher density of
        # points closer to the poles due to uniform grid sampling of full-space
        inc = (np.pi/2) - obj.dirs[:, 1]  # [nDir x 1]
        Ninc = np.prod(obj.ele.shape)
        Nazi = np.prod(obj.azi.shape)
        m = np.arange(Ninc/2)
        m = m[:, None].T  # (1,M)
        w = np.sum(np.sin(inc[:, None] * (2*m+1)) / (2*m+1), axis=1) *\
            np.sin(inc) * 2 / (Ninc*Nazi)  # (nDir,1)
        w = w / np.sum(w)
        return w

    def prepare_iso_weights(obj):
        # DESCRIPTION: calculates the beamformer weights for all directions and
        #              frequencies (to be called once and be used several)
        nFreq = obj.nFreq
        nChan = obj.nChan
        nOut = obj.nOut
        nDir = obj.nDir
        ...
        # conjugate weights
        obj.w_conj = np.zeros((nChan, nFreq, nDir, nOut), dtype=np.complex64)
        # number of interations to wait before updating progress print
        # (keep it high as the loop is fast)
        print_cycle = 50
        for di in range(nDir):
            if (di % print_cycle == 0):
                print('\r', 'Preparing weights %%%2.2f ' % (100*di/nDir),
                      end='\r')
            # unnormalized RTF [nFreq x nChan]
            h = np.squeeze(obj.ATF[:, di, :])
            for fi in range(nFreq):
                R = np.squeeze(obj.R_iso[fi, :, :])  # [nChan x nChan]
                for oi in range(nOut):
                    # normalized RTF [nFreq x nChan]
                    rtf = h / h[:, outChan_ind[oi]].reshape(-1, 1)
                    obj.w_conj[:, fi, di, oi] = np.conj(
                        obj.get_mvdr_weights(R, rtf[fi, :]))  # [nChan x 1]
        print('\r', 'Preparing weights %%%2.2f - Done!' % (100), end='\n')

    def get_angle_between(obj, ref, x, unit='radian'):
        # DESCRIPTION: calculates the angular separation between the directions
        #              (in either cartesian or DOA format)
        # *** INPUTS ***
        # ref   (ndarray)  reference direction(s) [1x2] or [Nx2]
        #                  (azimuth, elevation) or [1x3] or [Nx3] (x, y, z)
        # x     (ndarray)  subject direction(s) [Nx2] (azimuth,elevation)
        #                  or [Nx3] (x, y, z)
        # unit  (str)  unit of DOA if ref and x inputs are DOAs
        #              'radian' or 'degree'
        # *** OUTPUTS ***
        # a     (ndarray) angle between direction(s) [Nx1] in the same unit
        #                 as input
        if ref.shape[0] != x.shape[0]:
            # there is one ref and multiple subject.
            # repeat ref to match the number of subjects
            ref = np.matlib.repmat(ref, x.shape[0], 1)
        if ref.shape[1] > 2:
            # inputs are in cartesian. convert to spherical angles
            # [N x 2] (az,el)
            ref = np.array(obj.cart2sph(
                ref[:, 0], ref[:, 1], ref[:, 2]))[[0, 1], :].T
            x = np.array(obj.cart2sph(x[:, 0], x[:, 1], x[:, 2]))[[0, 1], :].T
        else:
        ...
        a = np.sin(dlat/2) ** 2 +\
            np.cos(x[:, 1]) * np.cos(ref[:, 1]) * np.sin(dlon/2) ** 2
    ...
    def process_stft(obj, X, target_doas):
        ...
        # target_doas  (ndarray) target DOA (azimuth,elevation) in radians
        #                        [nFrame x 2]
        # *** OUTPUTS ***
        # Y  (ndarray)  enhanced signal in the stft domain
        #               [nOut x nFreq x nFrame]
        # w  (ndarray)  beamformer conjugate weights in the stft domain
        #               [nChan x nFreq x nFrame x nOut]
        nChan, nFreq, nFrame = X.shape  # [nChan x nFreq x nFrame]
        nOut = len(obj.out_chan)
        # beamforning STFT conj weights [nChan x nFreq x nFrame x nOut]
        w = np.zeros((nChan, nFreq, nFrame, nOut), dtype=np.complex64)
        # enhanced signal in STFT domain [nOut x nFreq x nFrame]
        Y = np.zeros((nOut, nFreq, nFrame), dtype=np.complex64)
        # initize the array length to store index of target direction per frame
        obj.target_dir_ind = [0] * nFrame
        ...
        for framei in range(nFrame):
            if (framei % print_cycle == 0):
                print('\r', 'Processing %%%2.2f ' % (100*framei/nFrame),
                      end='\r')
            dist = obj.get_angle_between(target_doas[framei, :], obj.dirs)
            # nearest neighbour to target doa
            di = np.where(dist == dist.min())[0][0]
            obj.target_dir_ind[framei] = di
            for oi in range(nOut):
                # read conj weights from pre-stored directory obj.w_conj
                # [nChan x nFreq x nDir x nOut]
                w[:, :, framei, oi] = obj.w_conj[:, :, di, oi]
                Y[oi, :, framei] = np.sum(
                    X[:, :, framei] * w[:, :, framei, oi], axis=0)
        obj.w_stft = w
    ...
    def get_mvdr_weights(obj, R, d):
        ...
        # *** INPUTS ***
        # R  (ndarray)  covariance matrix [nChan x nChan]
        # d  (ndarray)  steering vector or Relative Transfer Function (RTF)
        #               [nChan x 1]
        # *** OUTPUTS ***
        # w  (ndarray)  beamformer conjugate weights in the stft domain
        #               [nChan x 1]
    ...
    def do_stft(obj, x):
        ...
        f, t, X = signal.stft(
            x, fs=obj.fs, window=obj.stft_params['window'],
            nperseg=round(obj.fs*obj.stft_params['winL']),
            noverlap=round(obj.fs*obj.stft_params['stepL']))
        obj.f = f
        obj.t = t
        return X, f, t
    ...
    def process_file(obj, in_file, out_file=[], start_t=0.0, duration=None):
        # DESCRIPTION: applies beamforming on an audio file and
        #              optionally write the enhanced file as an audio file
        # *** INPUTS ***
        # in_file  (str)  path to the input audio file
        # out_file (str)  optional path to the output audio file to write.
        #                 [] avoids writing file and only outputs the signal
        # *** OUTPUT ***
    ...
    def process_signal(obj, x, start_t=0.0, target_doa=[]):
        ...
        # *** INPUTS ***
        # x          (ndarray) signal in time doamin [nChan x nSample]
        # start_t    (float)   time in sec representing the start of
        #                      the input signal (used for target DOA reading)
        # target_doa (ndarray) target DOA (azimuth,elevation) in radians
        #                      [nFrame x 2]
        # *** OUTPUT ***
    ...
    def read_audio(obj, in_file, fs=None, start_t=0.0, duration=None, mics=[]):
        ...
        # fs       (int)   optional - output sample rate in Hz,
        #                  default: original sample rate
        # start_t  (float) optional - start time of segment in sec,
        #                  default: beginning of the file
        # duration (float) optional - duration of segment in sec,
        #                  default: entirely to the end of the file
        # mics     (list)  optional - mics/channels subset (indexing from 1),
        #                  default: all channels
        # *** OUTPUTS ***
        # y   (ndarray) output array audio signal [nChan x nSample]
        # fs  (int)     output sample rate in Hz
        y, fs = librosa.load(
            in_file, sr=fs, mono=False, offset=start_t, duration=duration)
        if mics:
            y = y[np.array(mics)-1, :]
        return y, fs
    ...
    def get_beampattern_from_weights(obj, start_t=0.0, duration=[]):
        # DESCRIPTION: returns the time-domain audio signal
        # *** INPUTS ***
        # start_t   (float)  time for a single frame or start time in sec
        #                    of animated beam pattern
        # duration  (float)  duration time in sec for animated beam pattern
        #                    (use 0 for single frame)
        # *** OUTPUTS ***
        # B     (ndarray)  Beam pattern [nFreq x nDOA x nFrame]
        # f     (ndarray)  frequency vector in Hz [nFreq x 1]
        # doas  (ndarray)  DOAs axis in degrees [nDOA x 2] (azimuth,elevation).
        #                  note: elevations are 0 (horizontal plane)
        # t     (ndarray)  time vector in sec [nFrame x 1]
        if isinstance(duration, list):
            duration = obj.t[-1] - obj.t[0]
        frame_ind = np.argmin(
            abs(obj.t[:, None] - np.array([start_t, start_t+duration])),
            axis=0)  # start and stop frame [1, 2]
        ...
        for n in range(doas.shape[0]):
            ...
            # nearest neighbour to target doa
            doas_i.append(np.where(dist == dist.min())[0][0])
        doas_i = np.array(doas_i)
        # obj.w_stft: [nChan x nFreq x nFrame x nOut]
        # w: [nChan x nFreq x nFrame]
        out_chan_ind = 0
        w = np.squeeze(
            obj.w_stft[:, :, frame_ind[0]:frame_ind[1]+1, out_chan_ind])
        if len(w.shape) == 2:
        ...
        # H @ w : [nChan x nFreq x nDOA x 1] @ [nChan x nFreq x 1 x nFrame]
        #         -> [nChan x nFreq x nDOA x nFrame]
        B = np.squeeze(np.sum(H @ w, axis=0))  # [nFreq x nDOA x nFrame]

        # normalization by ATF at the target direction at every time frame
        # (to get flat response at the target)
        target_dir_i = np.array(
            obj.target_dir_ind)[frame_ind[0]:frame_ind[1]+1]  # [nFrame x 1]
        # ATF [nFreq x nDir x nChan] -> [nFreq x nFrame]
        H_target = obj.ATF[
            :, target_dir_i, np.array(obj.out_chan[out_chan_ind])-1]
        # [nDOA x nFreq x nFrame]
        H_target = np.matlib.tile(H_target, (nDOA, 1, 1))
        # [nFreq x nDOA x nFrame]
        H_target = np.transpose(H_target, axes=[1, 0, 2])
    ...
    def plot_beampattern(obj, start_t=0.0, duration=0):
        ...
        # *** INPUTS ***
        # start_t  (float) time for a single frame or start time in sec of
        #                  animated beam pattern
        # duration (float) duration time in sec for animated beam pattern
        #                  (use 0 for single frame)
        B, f, doas, t = obj.get_beampattern_from_weights(
        ...
        for frame in range(B.shape[2]):
            plt.pcolormesh(
                doas[:, 0], f, 20*np.log10(abs(np.squeeze(B[:, :, frame]))),
                shading='auto', cmap='magma', vmin=-30, vmax=0)
            plt.title('Time: %2.2f s, Target Azimuth: %d deg'
                      % (t[frame], az[frame]))
