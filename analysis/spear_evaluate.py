...
# 2022 May - Sina Hafezi - initial version including all metrics:
#            MBSTOI, STOI, ESTOI, PESQ, PESQ-NB, SegSNR, fwSegSNR,
#            SI-SDR, SDR, ISR, SAR, HASPI
# 2022 Jul - Pierre Guiraud - updated into a function to be used in
#            the batch_master script
# 2022 Jul - Sina Hafezi - making the paths concatenation robust to
...
def compute_metrics(x_proc, x_ref, fs_ref, cols):
    ...
    if any(metric_name.split(' (')[0] in spmetrics_names
           for metric_name in cols):
        # TODO: choose specific metrics computed
        #       rather than always doing them all
        ...
        try:
            ...
            # Left
            SM_scores.append(spmetrics(x_proc[:, 0], x_ref[:, 0], rate=fs))
            print('compute_metrics: Left done')
            # Right
            SM_scores.append(spmetrics(x_proc[:, 1], x_ref[:, 1], rate=fs))
    ...
    # (some are already calculated/some to be calculated)
    scores = []
    for m in cols:
        print(f'{m}')
        try:
            if m == 'MBSTOI':  # stereo-based metric (get it once)
                mm = m
                score = mbstoi.mbstoi(
                    x_ref[:, 0], x_ref[:, 1], x_proc[:, 0], x_proc[:, 1], fs)
                ...
                if mm == 'STOI':
                    score = stoi(
                        x_ref[:, cc], x_proc[:, cc], fs, extended=False)
                elif mm == 'ESTOI':
                    score = stoi(
                        x_ref[:, cc], x_proc[:, cc], fs, extended=True)
                elif mm == 'SDR':
                    if SM_error:
                        raise ValueError
...
def spear_evaluate(spear_root, proc_dir, segments_file, save_path,
    ...
    if choose_set not in choose_set_allowed_values:
        print(f'Expected {choose_set} to be '
              f'one of {choose_set_allowed_values}')
        sys.exit()
    ...
    if save_path.is_file():
        print(f'File already exists at {save_path}. '
              f'Please choose a different filename or move the old one '
              f'to avoid loss of data.')
        sys.exit()

    # defines the time periods for each file
    # where it is valid to compute metrics
    ...
    cols = ['%s (%s)' % (x, y) for x in metrics for y in side_str]
    if isMBSTOI:
    ...
    for n in range(nSeg):
        ...
        proc_file_name = 'D%d_S%d_M%02d_ID%d.wav' % (
            dataset, session, minute, target_ID)
        ref_file_name = 'ref_D%d_S%d_M%02d_ID%d.wav' % (
            dataset, session, minute, target_ID)
        ...
        if not proc_file.is_file():
            ...
            ref_file = Path(ref_root, f'Dataset_{dataset}', 'Reference_Audio',
                            f'Session_{session}', f'{minute:02d}',
                            ref_file_name)
            if not ref_file.is_file():
                print(f'Expected reference file at {ref_file} is missing. '
                      f'Attempt to save before aborting...')
                ended_early = True
                break

            # debugging
            print(f'{proc_file_name}: sample_start: {sample_start}, '
                  f'sample_stop: {sample_stop}')

            # read in the audio
            x_proc, fs_proc = sf.read(
                proc_file, start=sample_start, stop=sample_stop+1)
            if len(x_proc.shape) < 2:
                # mono to stereo conversion if needed
                x_proc = np.tile(x_proc, (2, 1)).transpose()

            x_ref, fs_ref = sf.read(
                ref_file, start=sample_start, stop=sample_stop+1)
            if len(x_ref.shape) < 2:
                # mono to stereo conversion if needed
                x_ref = np.tile(x_ref, (2, 1)).transpose()
            ...
            metric_vals_df = pd.DataFrame(
                [chunk_info + scores], columns=cols_csv)

            if not header_was_written:
                metric_vals_df.to_csv(
                    save_path, index=False)
                header_was_written = True
            else:
                metric_vals_df.to_csv(
                    save_path, index=False, header=False, mode='a')
...
if __name__ == '__main__':
    ...
    parser.add_argument("segments_file",
                        help="csv file containg list of chunks "
                             "where metrics are valid")
    ...
    parser.add_argument("-m", "--metrics",
                        help="list a subset of metrics to compute "
                             "(default is to compute them all)",
                        default=None, nargs='+')
    ...
    spear_evaluate(
        args.input_root, args.proc_dir, args.segments_file, args.save_path,
