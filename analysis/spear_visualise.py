...
# 2022 Jun - Sina Hafezi - initial version, boxplot of absolute and
#            relative metrics (vs passthrough)
...
def spear_visualise(output_dir, metrics_ref, metrics_proc,
                    reference_name='passthrough', method_name='baseline'):
    # ensure the output directory is present before we start running anything
    # - to allow running supplementary results files which already exist
    ...
    seg_pass = seg_pass.drop(
        ['global_index', 'file_name', 'chunk_index'], axis=1)

    seg_proc = pd.read_csv(metrics_proc)
    seg_proc = seg_proc.drop(
        ['global_index', 'file_name', 'chunk_index'], axis=1)
    ...
    for n in range(nCol):
        ...
        if metric_name not in visited_metrics:
            ...
            if '(' in col_name:
                ...
                # opposite side
                oside_name = sides[int(not bool(sides.index(side_name)))]
                # index of opposite side
                n2 = cols.index('%s (%s)' % (metric_name, oside_name))

                # [nChunk x 2]
                s_pass = np.array([seg_pass[cols[n]], seg_pass[cols[n2]]]).T
                s_proc = np.array([seg_proc[cols[n]], seg_proc[cols[n2]]]).T

                # Remove NaN values
                nan_ids = np.where(
                    np.isnan(s_pass[:, 0]) | np.isnan(s_pass[:, 1]) |
                    np.isnan(s_proc[:, 0]) | np.isnan(s_proc[:, 1]))
                s_pass = np.array([
                    np.delete(s_pass[:, 0], nan_ids),
                    np.delete(s_pass[:, 1], nan_ids)]).T
                s_proc = np.array([
                    np.delete(s_proc[:, 0], nan_ids),
                    np.delete(s_proc[:, 1], nan_ids)]).T

                # making sure the order of side is L then R
                if side_name == 'R':
...
if __name__ == '__main__':
    ...
    parser.add_argument("-r", "--reference_name",
                        help="reference algorithm used: [passthrough] "
                             "by default",
                        default='passthrough')
    parser.add_argument("-m", "--method_name",
                        help="enhancement algorithm used: [baseline] "
                             "by default",
                        default='baseline')
    ...
    spear_visualise(
        args.output_root, args.metrics_ref, args.metrics_proc,
        reference_name=args.reference_name, method_name=args.method_name)
