...
from SPEAR import SPEAR_Data
from Processor import SPEAR_Processor


def spear_enhance(input_root, output_dir, list_cases=[],
    ...
    if method_name not in method_name_allowed_values:
        raise ValueError(
            f'method_name must be one of {method_name_allowed_values}')

    # ensure the output directory is present before we start running anything
    # - to allow running supplementary results files which already exist will
    #   not be replaced
    ...
    if method_name == 'baseline':
        ...
        proc = SPEAR_Processor(
            sp.get_all_AIRs(), fs=fs, mics=mics, out_chan=out_chan)
...
if __name__ == '__main__':
    ...
    spear_enhance(
        args.input_root, args.output_root, list_cases=list_cases,
